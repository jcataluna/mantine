...
- `align` – defines line and bullets position relative to content: `left`, `right`, or `center`
- `compact` – if set, reduces spacing between items
...
Set `align="center"` to display the timeline with a centered line. Use the `side` prop
on `Timeline.Item` to control which side (`left` or `right`) the content appears on.
Items default to the right side when no `side` prop is specified.

<Demo data={TimelineDemos.centerAlign} />

## Compact mode

Set `compact` prop to reduce spacing between timeline items. This works with all
alignment modes.

<Demo data={TimelineDemos.compact} />

## Timeline.Group

Use `Timeline.Group` to display parallel content on both sides of the timeline simultaneously.
This is useful for showing related events that happen at the same time. `Timeline.Group`
accepts `left` and `right` props for the content on each side, and an optional `bullet` prop.

Note: `Timeline.Group` is intended for use with `align="center"`.

<Demo data={TimelineDemos.group} />
